      ...
      <div className="max-w-md w-full border border-border bg-card p-10 text-center">
        ...
        {state === "paid" && (
          <>
            ...
            <p className="text-sm text-muted-foreground mt-2">Your job is now scheduled. The professional has been notified.</p>
            <Link to="/dashboard/payments"><Button data-testid="goto-payments" className="mt-6 rounded-none bg-accent hover:bg-accent/90 text-white w-full">View my payments</Button></Link>
          </>
        )}
        {state === "pending" && (
          <>
            <h1 className="font-display font-bold text-xl">Almost there</h1>
            <p className="text-sm text-muted-foreground mt-2">Payment is processing — check your payments page in a moment.</p>
            <Link to="/dashboard/payments"><Button data-testid="goto-payments-pending" className="mt-6 rounded-none w-full">My payments</Button></Link>
          </>
        )}
        {state === "missing" && (
          <>
